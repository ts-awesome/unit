...
```ts
const AtomicMassUnitUnified = scale("Atomic mass unit, unified", "u", mul(unit(1, kilogram), 1.66053904e-27));
```
...
```ts
const AtomicUnitOfMassElectronRestMass = scale("Atomic unit of mass, electron rest mass", "me", mul(unit(1, kilogram), 9.10938356e-31));
```
...
[Source](http://conversion.org/mass/bag-coffee/)
...
```ts
const BagCoffee = scale("Bag (coffee)", "bag (coffee)", mul(unit(1, kilogram), 60));
```
...
```ts
const Bag_PortlandCement = scale("Bag (Portland cement)", "bag (Portland cement)", mul(unit(1, PoundAvoirdupois), 94));
```
...
```ts
const Barge = scale("Barge", "barge", mul(unit(1, TonShort), 22.5));
```
...
```ts
const Carat = scale("Carat", "kt", mul(unit(1, Grain), 3.1666666666666665));
```
...
```ts
const CaratMetric = scale("Carat (metric)", "ct", mul(unit(1, Milligram), 200));
```
...
```ts
const Centigram = scale("Centigram", "cg", mul(unit(1, kilogram), 0.00001));
```
...
```ts
const Clove = scale("Clove", "clove", mul(unit(1, PoundAvoirdupois), 8));
```
...
```ts
const Crith = scale("Crith", "crith", mul(unit(1, Milligram), 89.9349));
```
...
```ts
const Dalton = scale("Dalton", "Da", mul(unit(1, kilogram), 1.66053904e-27));
```
...
```ts
const Decagram = scale("Decagram", "dag", mul(unit(1, kilogram), 0.01));
```
...
```ts
const DramApothecaryTroy = scale("Dram (apothecary; troy)", "dr t", mul(unit(1, Grain), 60));
```
...
```ts
const DramAvoirdupois = scale("Dram (avoirdupois)", "dr av", "dr", mul(unit(1, Grain), 27.34375));
```
...
```ts
const Gamma = scale("Gamma", "γ", mul(unit(1, Microgram), 1));
```
...
```ts
const Grain = scale("Grain", "gr", mul(unit(1, PoundAvoirdupois), 0.00014285714285714287));
```
...
```ts
const Gram = scale("Gram", "g", mul(unit(1, kilogram), 0.001));
```
...
```ts
const Grave = scale("Grave", "gv", mul(unit(1, kilogram), 1));
```
...
```ts
const Hectogram = scale("Hectogram", "hg", mul(unit(1, kilogram), 0.1));
```
...
```ts
const HundredweightLong = scale("Hundredweight (long)", "long cwt", "cwt", mul(unit(1, PoundAvoirdupois), 112));
```
...
> **Hundredweight (short); cental** is  _mass_ unit, symbol: [sh cwt]. Definition of 1 hundredweight (short); cental ≡ 100 lb av . ≡100 × 0.45359237 kg ≡ 45.359237 kg.
...
```ts
const HundredweightShortCental = scale("Hundredweight (short); cental", "sh cwt", mul(unit(1, PoundAvoirdupois), 100));
```
...
```ts
const Kip = scale("Kip", "kip", mul(unit(1, PoundAvoirdupois), 1000));
```
...
```ts
const Mark = scale("Mark", "mark", mul(unit(1, OunceApothecaryTroy), 8));
```
...
```ts
const Microgram = scale("Microgram", "µg", mul(unit(1, kilogram), 1e-9));
```
...
```ts
const Milligram = scale("Milligram", "mg", mul(unit(1, kilogram), 0.000001));
```
...
```ts
const Mite = scale("Mite", "mite", mul(unit(1, Grain), 0.05));
```
...
```ts
const MiteMetric = scale("Mite (metric)", "mite (metric)", mul(unit(1, Gram), 0.05));
```
...
```ts
const Nanogram = scale("Nanogram", "ng", mul(unit(1, kilogram), 1e-12));
```
...
```ts
const OunceApothecaryTroy = scale("Ounce (apothecary; troy)", "oz t", mul(unit(1, PoundTroy), 0.08333333333333333));
```
...
```ts
const OunceAvoirdupois = scale("Ounce (avoirdupois)", "oz av", "oz", mul(unit(1, PoundAvoirdupois), 0.0625));
```
...
> **Ounce (US food nutrition labelling)** is  _mass_ unit, symbol: [oz (US)]. Definition of 1 ounce (US food nutrition labelling) ≡ 28 g. The ounce (abbreviated oz) is a unit of mass used in most British derived customary systems of measurement..
...
```ts
const Ounce_USFoodNutritionLabelling = scale("Ounce (US food nutrition labelling)", "oz (US)", mul(unit(1, Gram), 28));
```
...
```ts
const Pennyweight = scale("Pennyweight", "dwt", mul(unit(1, OunceApothecaryTroy), 0.05));
```
...
```ts
const Point = scale("Point", "point", mul(unit(1, CaratMetric), 0.01));
```
...
```ts
const PoundAvoirdupois = scale("Pound (avoirdupois)", "lb av", "lb", "lbs", mul(unit(1, kilogram), 0.45359237));
```
...
```ts
const PoundMetric = scale("Pound (metric)", "pound (metric)", mul(unit(1, Gram), 500));
```
...
[Source](http://conversion.org/mass/pound-troy/)
...
```ts
const PoundTroy = scale("Pound (troy)", "lb t", mul(unit(1, Grain), 5760));
```
...
```ts
const QuarterImperial = scale("Quarter (imperial)", "quarter (imp)", mul(unit(1, PoundAvoirdupois), 28));
```
...
```ts
const QuarterInformal = scale("Quarter (informal)", "quarter (informal)", mul(unit(1, TonShort), 0.25));
```
...
```ts
const QuarterLongInformal = scale("Quarter, long (informal)", "quarter, long (informal)", mul(unit(1, TonLong), 0.25));
```
...
```ts
const QuintalMetric = scale("Quintal (metric)", "q", mul(unit(1, kilogram), 100));
```
...
```ts
const ScrupleApothecary = scale("Scruple (apothecary)", "s ap", mul(unit(1, Grain), 20));
```
...
```ts
const Sheet = scale("Sheet", "sheet", mul(unit(1, PoundAvoirdupois), 0.0014285714285714286));
```
...
```ts
const SlugGeepoundHyl = scale("Slug; geepound; hyl", "slug", mul(unit(1, g0), unit(1, PoundAvoirdupois), pow(unit(1, second), 2), pow(unit(1, Foot_International), -1), 1));
```
...
```ts
const Stone = scale("Stone", "st", mul(unit(1, PoundAvoirdupois), 14));
```
...
```ts
const TonAssayLong = scale("Ton, assay (long)", "AT", mul(unit(1, Milligram), unit(1, TonLong), pow(unit(1, OunceApothecaryTroy), -1), 1));
```
...
```ts
const TonAssayShort = scale("Ton, assay (short)", "sh AT", mul(unit(1, Milligram), unit(1, TonShort), pow(unit(1, OunceApothecaryTroy), -1), 1));
```
...
```ts
const TonLong = scale("Ton, long", "ton", mul(unit(1, PoundAvoirdupois), 2240));
```
...
```ts
const TonShort = scale("Ton, short", "sh tn", mul(unit(1, PoundAvoirdupois), 2000));
```
...
```ts
const Tonne = scale("Tonne", "t", mul(unit(1, kilogram), 1000));
```
...
```ts
const Wey = scale("Wey", "wey", mul(unit(1, Stone), 18));
```
