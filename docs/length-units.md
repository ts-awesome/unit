...
```ts
const angstrom = scale("ångström", "Å", mul(unit(1, metre), 1e-10));
```
...
```ts
const AstronomicalUnit = scale("Astronomical unit", "AU", mul(unit(1, metre), 149597870700));
```
...
```ts
const BohrAtomicUnitOfLength = scale("Bohr, atomic unit of length", "a₀", mul(unit(1, metre), 5.29e-11));
```
...
```ts
const CableLengthImperial = scale("Cable length (imperial)", "cable (imp)", mul(unit(1, Foot_International), 608));
```
...
```ts
const CableLength_International = scale("Cable length (International)", "cable", mul(unit(1, NauticalMileInternational__nmi), 0.1));
```
...
```ts
const CableLength_US = scale("Cable length (US)", "cable (US)", mul(unit(1, Foot_International), 720));
```
...
```ts
const Centimetre = scale("Centimetre", "cm", mul(unit(1, metre), 0.01));
```
...
```ts
const Chain = scale("Chain", "ch", mul(unit(1, Foot_US_Survey), 66));
```
...
```ts
const Decimetre = scale("Decimetre", "dm", mul(unit(1, metre), 0.1));
```
...
> **Dekametre** is metric _length_ unit, symbol: [dam]. Definition of 1 dekametre  ≡10 m. .
...
```ts
const Dekametre = scale("Dekametre", "dam", mul(unit(1, metre), 10));
```
...
```ts
const Fathom = scale("Fathom", "ftm", mul(unit(1, Foot_International), 6));
```
...
```ts
const Fermi = scale("Fermi", "fm", mul(unit(1, metre), 1e-15));
```
...
```ts
const Finger = scale("Finger", "finger", mul(unit(1, Inch_International), 0.875));
```
...
```ts
const FingerCloth = scale("Finger (cloth)", "finger (cloth)", mul(unit(1, Inch_International), 4.5));
```
...
```ts
const Foot_International = scale("Foot (International)", "ft", mul(unit(1, metre), 0.3048));
```
...
```ts
const Foot_US_Survey = scale("Foot (US Survey)", "ft (US)", mul(unit(1, metre), 0.3048006096012192));
```
...
```ts
const FootMetric = scale("Foot, metric", "mf", mul(unit(1, metre), 0.31622776601683794));
```
...
```ts
const FootMetricLong = scale("Foot, metric (long)", "lmf", mul(unit(1, metre), 0.3333333333333333));
```
...
```ts
const FootMetricShort = scale("Foot, metric (short)", "smf", mul(unit(1, metre), 0.3));
```
...
```ts
const Furlong = scale("Furlong", "fur", mul(unit(1, Foot_International), 660));
```
...
```ts
const Hand = scale("Hand", "hand", mul(unit(1, Inch_International), 4));
```
...
```ts
const Hectometre = scale("Hectometre", "hm", mul(unit(1, metre), 100));
```
...
```ts
const Inch_International = scale("Inch (International)", "in", mul(unit(1, Foot_International), 0.08333333333333333));
```
...
```ts
const Kilometre = scale("Kilometre", "km", mul(unit(1, metre), 1000));
```
...
```ts
const LeagueLand = scale("League (land)", "lea", mul(unit(1, Foot_US_Survey), 5280));
```
...
```ts
const LightDay = scale("Light-day", "light-day", mul(unit(1, LightHour), 24));
```
...
```ts
const LightHour = scale("Light-hour", "light-hour", mul(unit(1, LightMinute), 60));
```
...
```ts
const LightMinute = scale("Light-minute", "light-minute", mul(unit(1, LightSecond), 60));
```
...
```ts
const LightSecond = scale("Light-second", "light-second", mul(unit(1, SpeedOfLight), unit(1, second), 1));
```
...
```ts
const LightYear = scale("Light-year", "ly", mul(unit(1, LightDay), 365.25));
```
...
> **Line** is  _length_ unit, symbol: [ln]. Definition of 1 line ≡  1⁄12 in. .
...
```ts
const Line = scale("Line", "ln", mul(unit(1, Inch_International), 0.08333333333333333));
```
...
```ts
const Link_GunterS_SurveyorS = scale("Link (Gunter's; Surveyor's)", "lnk", mul(unit(1, Inch_International), 7.92));
```
...
```ts
const Link_RamsdenS_EngineerS = scale("Link (Ramsden's; Engineer's)", "lnk (Ramsden)", mul(unit(1, Foot_International), 1));
```
...
```ts
const Mickey = scale("Mickey", "mickey", mul(unit(1, Inch_International), 0.005));
```
...
```ts
const Micrometre = scale("Micrometre", "µm", mul(unit(1, metre), 0.000001));
```
...
[Source](http://conversion.org/length/micron/)
...
```ts
const Micron = scale("Micron", "µ", mul(unit(1, metre), 0.000001));
```
...
```ts
const Mil_SwedenAnd_Norway = scale("Mil (Sweden and Norway)", "mil (Sweden and Norway)", mul(unit(1, Kilometre), 10));
```
...
```ts
const MilThou = scale("Mil; thou", "mil", mul(unit(1, Inch_International), 0.001));
```
...
```ts
const MileInternational = scale("Mile (international)", "mi", mul(unit(1, Foot_International), 5280));
```
...
```ts
const MileTacticalOrData = scale("Mile (tactical or data)", "mile (tactical or data)", mul(unit(1, Foot_International), 6000));
```
...
```ts
const Mile_US_Survey = scale("Mile (US Survey)", "mi (US)", mul(unit(1, Foot_US_Survey), 5280));
```
...
```ts
const Millimetre = scale("Millimetre", "mm", mul(unit(1, metre), 0.001));
```
...
```ts
const NailCloth = scale("Nail (cloth)", "nail (cloth)", mul(unit(1, Inch_International), 2.25));
```
...
```ts
const Nanometre = scale("Nanometre", "nm", mul(unit(1, metre), 1e-9));
```
...
```ts
const NauticalLeague = scale("Nautical league", "NL", mul(unit(1, NauticalMileInternational__nmi), 3));
```
...
```ts
const NauticalMile_Admiralty = scale("Nautical mile (Admiralty)", "NM (Adm)", mul(unit(1, Foot_International), 6080));
```
...
```ts
const NauticalMileInternational = scale("Nautical mile (international)", "NM", mul(unit(1, metre), 1852));
```
...
```ts
const Pace = scale("Pace", "pace", mul(unit(1, Foot_International), 2.5));
```
...
```ts
const Palm = scale("Palm", "palm", mul(unit(1, Inch_International), 3));
```
...
> **Parsec** is  _length_ unit, symbol: [pc]. Definition of 1 parsec ≡ 3.085677581e16 m. Distance of star with parallax shift of one arc second from a base of one astronomical unit .
...
```ts
const Parsec = scale("Parsec", "pc", mul(unit(1, metre), 30856775810000000));
```
...
[Source](http://conversion.org/length/pica-postscript/)
...
```ts
const Pica_PostScript = scale("Pica (PostScript)", "pica (PostScript)", mul(unit(1, Point_PostScript), 12));
```
...
```ts
const PicometreOldBicronStigma = scale("Picometre (old: bicron, stigma)", "pm (old)", mul(unit(1, metre), 1e-12));
```
...
```ts
const Pikometre = scale("Pikometre", "pm", mul(unit(1, metre), 1e-12));
```
...
```ts
const Point_American_English = scale("Point (American, English)", "pt (US)", mul(unit(1, Inch_International), 0.013836617223821119));
```
...
```ts
const Point_Didot_European = scale("Point (Didot; European)", "pt (Didot)", mul(unit(1, Centimetre), 0.03759398496240601));
```
...
> **Point (PostScript)** is  _length_ unit, symbol: [pt]. Definition of 1 point (PostScript) ≡  1⁄72 in . .
...
```ts
const Point_PostScript = scale("Point (PostScript)", "pt", mul(unit(1, Inch_International), 0.013888888888888888));
```
...
```ts
const Point_TeX = scale("Point (TeX)", "pt (TeX)", mul(unit(1, Inch_International), 0.013837000138370002));
```
...
```ts
const Quarter = scale("Quarter", "quarter", mul(unit(1, Yard_International), 0.25));
```
...
```ts
const Shaku_Japan = scale("Shaku (Japan)", "shaku (Japan)", mul(unit(1, metre), 0.30303030303030304));
```
...
```ts
const Spat = scale("Spat", "spat", mul(unit(1, metre), 1000000000000));
```
...
```ts
const Twip = scale("Twip", "twp", mul(unit(1, Inch_International), 0.0006944444444444445));
```
...
[Source](http://conversion.org/length/x-unit-siegbahn/)
...
```ts
const XUnitSiegbahn = scale("X unit; siegbahn", "xu", mul(unit(1, metre), 1.0021e-13));
```
...
```ts
const Yard_International = scale("Yard (International)", "yd", mul(unit(1, Inch_International), 36));
```
...
```ts
const Zoll = scale("Zoll", "zoll", mul(unit(1, Millimetre), 25.4));
```
